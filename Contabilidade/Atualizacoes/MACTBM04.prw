...
±±ºPrograma  ³MACTBM04  ºAutor  ³Tarcísio Silva      º Data ³  02/02/18   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Limpa fleg da SF1 - Documento de Entrada.  				  º±±
...
User Function MACTBM04()
if !RetCodUsr()$GetMv("MV_XCTBSF1",,"000287")
	MsgAlert("Usuário sem premissão - MV_XCTBSF1!","Atenção")
	return()
endif
...
Aadd(aTypes,{ "Iif( !Eof(),!empty(F1_DTLANC), .F. )"		,	"BR_VERDE" 	}	)
Aadd(aTypes,{ "Iif( !Eof(),empty(F1_DTLANC), .F. )"		,	"BR_VERMELHO" 	}	)
...
Private aRotina := { {"Pesquisar","AxPesqui",0,1} ,;
             {"Visualizar","AxVisual",0,2} ,;
             {"Sim/Nao","U_SIMNAOF1",0,3} ,;
             {"Legenda","U_LegAcert",0,4},;
             {"Processo em Massa","Processa({|| u_MACTBM06('SF1') },'Processando...')",0,5}}
...
Return
User Function SIMNAOF1

		reclock("SF1",.f.)
		replace F1_DTLANC 	with iif(!Empty(F1_DTLANC),cTod("  /  /   "),Date())
		msUnLock()
