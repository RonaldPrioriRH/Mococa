...
±±ºPrograma  ³MACTBM03  ºAutor  ³Tarcísio Silva      º Data ³  02/02/18   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Limpa fleg da SE1 - Contas a Receber.						  º±±
...
User Function MACTBM03
if !RetCodUsr()$GetMv("MV_XCTBSE1",,"000287")
	MsgAlert("Usuário sem premissão - MV_XCTBSE1!","Atenção")
	return()
endif
...
Private		aTypes     	:= {}
Aadd(aTypes,{ "Iif( !Eof(),E1_LA='S', .F. )"		,	"BR_VERDE" 	}	)
Aadd(aTypes,{ "Iif( !Eof(),(E1_LA='N' .OR. E1_LA=' '), .F. )"		,	"BR_VERMELHO" 	}	)
...
Private aRotina := { {"Pesquisar","AxPesqui",0,1} ,;
             {"Visualizar","AxVisual",0,2} ,;
             {"Sim/Nao","U_SIMNAOE1",0,3} ,;
             {"Legenda","U_LegAcert",0,4},;
             {"Processo em Massa","Processa({|| u_MACTBM06('SE1') },'Processando...')",0,5}}
...
Return
User Function SIMNAOE1

		reclock("SE1",.f.)
		replace E1_LA 	with IIF(E1_LA="S","N","S")
		msUnLock()
