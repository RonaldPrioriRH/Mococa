...
cString += '<dhEmi>'+ConvType(aNota[03])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")+'</dhEmi>'
	...
	//alterado Data Entrada/Saida diferente da emissão campo SC5
	//cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(aNota[07])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
	//Correção do erro array out of bounds [7] of [6]  on NFEIDE(NFESEFAZ.PRW) 07/12/2020 15:42:57 line : 6097 
	//na transmissão dos retornos, devido fonte RFATE025 só funcionar para  cTipo == "1" Saida e Não cTipo == "0",
	//mantive o fonte alterei apenas quando o tamanho do array for mair que 6 ou sejá vem do RFATE025 aNota[07] 

//cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",Iif(Len(aNota) > 6,ConvType(aNota[07]),ConvType(aNota[03]))+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",Iif(Len(aNota) > 8,ConvType(aNota[09]),ConvType(aNota[03]))+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))

/*
	Atenção com a posição do Array aNota, se aumentar tem que corrigir o trecho abaixo devido a alteração para Mococa da dhEmi e dhSaiEnt diferente da F2_EMISSAO estado de Minas Gerais
