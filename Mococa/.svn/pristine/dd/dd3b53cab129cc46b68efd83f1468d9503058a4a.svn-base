...
For nI := 1 To Len(aData)
	...
	If !DbSeek(xFilial("SA2")+cCodFaz)
		...
		If DbSeek(xFilial("SA2")+SubStr(cBuffer,4,7))
...
//		nVlrAna  := Val(SubStr(cBuffer,22,9))
	...
	If cAnaPro <> "NOFOUND"
		...
		If SA2->(found())
			dtIni := DTOS(MV_PAR03)
			dtFim := DTOS(MV_PAR04)
			cQuery := " SELECT ISNULL(MIN(ZLD_DTCOLE),'XXX') DTCOLP "
			cQuery += " FROM "+RetSqlName("ZLD")
			cQuery += " WHERE D_E_L_E_T_ = ' ' "
			cQuery += " AND ZLD_RETIRO = '" + SubStr(cCodFaz,1,6) + "' "
			cQuery += " AND ZLD_RETILJ = '" + SubStr(cCodFaz,7,2) + "' "
			//cQuery += " AND ZLD_COLANA = 'S' "
			cQuery += " AND ZLD_DTCOLE BETWEEN '" + dtIni + "' AND '" + dtFim + "'   "
...
//@025,010 say "Importacao realizada com sucesso! " PIXEL OF oDlgMsn
