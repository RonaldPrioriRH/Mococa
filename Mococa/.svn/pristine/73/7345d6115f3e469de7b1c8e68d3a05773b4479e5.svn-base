...
User Function XPQIPR050() 
	...
	Private lProduto   := .F.
    Private lLabmic := .F.
    Private cIdioma := "Portugês"//Portugês//Espanhol
    Private cGrupo := ""
...
Static Function A050Imp(lEnd,wnRel,cString,cTitulo)
	...
	For nContador := 1 To mv_par07
						...
						If QP1->QP1_TPCART <> "X"
							...
							If dbSeek(xFilial("QP7")+QPR->QPR_PRODUT+QPR->QPR_REVI+cRoteiro+QPR->QPR_OPERAC+QPR->QPR_ENSAIO) .AND. QPR->QPR_OPERAC $ mv_par13
								If QP7->QP7_CERTIF == "S"
									...
									@Li,01 PSAY u_funTraduz(QP1->QP1_DESCPO,cIdioma) //CONTAGEM TOTAL//QP1->QP1_DESCPO
						...
						Else
							...
							If dbSeek(xFilial()+QPR->QPR_PRODUT+QPR->QPR_REVI+cRoteiro+QPR->QPR_OPERAC+QPR->QPR_ENSAIO) .AND. QPR->QPR_OPERAC $ mv_par13
								If QP8->QP8_CERTIF == "S"
									...
									@Li,01 PSAY u_funTraduz(QP1->QP1_DESCPO,cIdioma)//QP1->QP1_DESCPO
		...
		Endif
			...
			@Li,001 PSAY "____________________________________________________________________________________________________________________________________"//cImpLinha
			Li++
...
//***********************************************************************************		
		...
		If dbSeek(cSeek)
			...
			If dbSeek(xFilial("QPD")+QPL->QPL_LAUDO)
				Li+=2
				@Li,01 PSAY STR0067+Replicate(".",15-Len(STR0067))+": "+CapitalAce(QPD->QPD_DESCPO)//u_funTraduz(QPD->QPD_DESCPO,cIdioma)
				Li++
		...
		@Li,01 PSAY "[ "+STR0062+" ]"
		Li++
		@Li,01 PSAY "[kg =  quilograma] [FQ = Físico-Quimico] [N.M.P. = Número mais provavel] [Mín. = Mínimo] [µg = micrograma]" 
		Li++
		@Li,01 PSAY "[MB = Microbiologia] [Máx. = Máximo] [U.F.C = Unidade formadora de colonia] [N.D. =  Não Detectado] [Col. = Coliformes]"
		Li++
		@Li,01 PSAY "[mg = miligramas] [cps = centipoise] [ml = mililitro] [µ = micro] [Un. = Unidade] [% = porcentagem] [g = gramas]"
		Li++
