...
±±ºPrograma  ³MACTBM05  ºAutor  ³Tarcísio Silva      º Data ³  02/02/18   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Limpa fleg da SF2 - Documento de Saída.	  				  º±±
...
User Function MACTBM05()
if !RetCodUsr()$GetMv("MV_XCTBSF2",,"000287")
	MsgAlert("Usuário sem premissão - MV_XCTBSF2!","Atenção")
	return()
endif
...
Private		aTypes     	:= {}
Aadd(aTypes,{ "Iif( !Eof(),!empty(F2_DTLANC), .F. )"		,	"BR_VERDE" 	}	)
Aadd(aTypes,{ "Iif( !Eof(), empty(F2_DTLANC), .F. )"		,	"BR_VERMELHO" 	}	)
...
Private aRotina := { {"Pesquisar","AxPesqui",0,1} ,;
             {"Visualizar","AxVisual",0,2} ,;
             {"Sim/Nao","U_SIMNAOF2",0,3} ,;
             {"Legenda","U_LegAcert",0,4},;
             {"Processo em Massa","Processa({|| u_MACTBM06('SF2') },'Processando...')",0,5}}
...
//Pergunte(cPerg,.F.)
//SetKey(123,{|| Pergunte(cPerg,.T.)}) // Seta a tecla F22 para acionamento dos parametros
...
Set Key 123 To // Desativa a tecla F22 do acionamento dos parametros


Return
User Function SIMNAOF2

		reclock("SF2",.f.)
		replace F2_DTLANC 	with iif(!Empty(F2_DTLANC),cTod("  /  /   "),Date())
		msUnLock()

Return
/*Static Function LegAcert()
