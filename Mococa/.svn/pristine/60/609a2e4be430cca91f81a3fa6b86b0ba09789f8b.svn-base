...
Begin Sequence
   ...
   EEN->(dbSeek(xFilial()+EEC->EEC_PREEMB))
   ...
   //Notify
   HEADER_P->AVG_C08_60 := EEN->EEN_IMPODE
   HEADER_P->AVG_C09_60 := EEN->EEN_ENDIMP
   HEADER_P->AVG_C10_60 := EEN->EEN_END2IM 
   
  // Consignee
   HEADER_P->AVG_C11_60 := EEC->EEC_XNOMCO
   HEADER_P->AVG_C12_60 := EEC->EEC_XENDCO
   HEADER_P->AVG_C13_60 := EEC->EEC_XCENDC
   ...
   //Total Containers
     HEADER_P->AVG_C03_30 := DecPoint(LTrim(Transf((EXL->EXL_QTD20+EXL->EXL_QTD40+EXL->EXL_QTD40H),cPict1)))
   ...
   cTotPesLi := DecPoint(Transf(EEC->EEC_PESLIQ,AvSx3("EE9_PSBRTO",AV_PICTURE)),AvSx3("EE9_PSBRTO",AV_DECIMAL))
   ...
   If !GetMv("MV_AVG0004",, .F.)
      //cTotPesBr := DecPoint(LTrim(Transf(nTotPesBr/*EEC->EEC_PESBRU*/,cPict2)),2)  //LGS-09/12/2014   
      cTotPesBr := DecPoint(Transf(nTotPesBr,AvSx3("EE9_PSBRTO",AV_PICTURE)),AvSx3("EE9_PSBRTO",AV_DECIMAL))
   Else
   ...
   While EE9->(!Eof() .And. EE9_FILIAL == xFilial("EE9")) .And.;
      ...
      */
      ...
      IF ! EE2->(Dbseek(xFilial("EE2")+"8"+"*"+EEC->EEC_IDIOMA+EE9->EE9_UNIDAD))
          MsgStop(STR0001+EE9->EE9_UNIDAD+"nào cadastrada em"+EEC->EEC_IDIOMA,STR0003) //"Unidade de medida "###"  "###"Aviso"
      Endif
      
      DETAIL_P->AVG_C02_20 := DecPoint(Transf(EE9->EE9_SLDINI,"@E 99,999,999,999.999")) + " " + AllTrim(EE2->EE2_DESCMA)     
      ...
      */ 
      ...
      If EEK->(dbSeek(xFilial("EEK")+OC_EMBA+EE9->EE9_EMBAL1))
         While EEK->(!EOF()) .and. EEK->(EEK_FILIAL+EEK_TIPO+EEK_CODIGO) == xFilial("EEK")+OC_EMBA+EE9->EE9_EMBAL1
            ...
            cUltEmb := EEK->EEK_EMB
            //LGS-09/12/2014-Deve sempre apresentar o total de embalagens
      ...
      Else
	     ...
	     If At("RACK",EE5->EE5_DESC) > 0
	        ...
	        */
	        nTotRacks += nQtdUltEmb 
	     Else
	        nTotRacks := nQtdUltEmb //LRS - 11/04/2014 - Caso não achar a palavra "RACK" no EE5_DESC Total de Rack recebe Quantidade Emb.
	     Endif
      ...
      */
      ...
      If(SB1->B1_PESO/SB1->B1_CONV) >= 1 
        cPackMoc:=DecPoint(Transf((SB1->B1_PESO/SB1->B1_CONV),cPict2))+"KG"
      Else
        cPackMoc:=DecPoint(Transf(((SB1->B1_PESO/SB1->B1_CONV)*1000),cPict1))+"G"     
      EndIF
      
      //Packaging
        DETAIL_P->AVG_C03_20 :=DecPoint(Transf(SB1->B1_CONV,cPict1)+"X"+cPackMoc)
      ...
      //Peso Liquido Total por item
      DETAIL_P->AVG_C04_20 := DecPoint(Transf(EE9->EE9_PSLQTO,AvSx3("EE9_PSLQTO",AV_PICTURE)),AvSx3("EE9_PSLQTO",AV_DECIMAL)) 
      
      // Peso Bruto Total
      DETAIL_P->AVG_C05_20 := DecPoint(Transf(EE9->EE9_PSBRTO,AvSx3("EE9_PSBRTO",AV_PICTURE)),AvSx3("EE9_PSBRTO",AV_DECIMAL))
      nTotPesBr += EE9->EE9_PSBRTO //LGS-09/12/2014
      
      //Total Embalagens/Pallet -> Qtde. de Embalagens mais Externas
      If EEC->(FieldPos("EEC_QTDEMB")) # 0 .AND. EEC->EEC_QTDEMB # 0  // GFP - 24/01/2014
         DETAIL_P->AVG_C08_20 := DecPoint(LTrim(Transf(EEC->EEC_QTDEMB,cPict1)))
      Else
         DETAIL_P->AVG_C08_20 := DecPoint(LTrim(Transf(nQtdUltEmb,cPict1)))
      EndIf  
      
      // Total Embalagens
       DETAIL_P->AVG_C06_20 := DecPoint(Transf(EE9->EE9_SLDINI,cPict1))  
      

      // Total Peças / Embalagens
      //DETAIL_P->AVG_C07_20 := DecPoint(Transf(EE9->EE9_QE,cPict1))
      //Total de itens por embalagem
        DETAIL_P->AVG_C07_20 := DecPoint(Transf((SB1->B1_CONV*EE9->EE9_SLDINI),cPict1))  

      // Descricao Produto 
      //cMemo := AA100Idioma(EE9->EE9_COD_I) //GFP - 29/05/2012 - Tratamento de idiomas. 
      ...
      //Alexandre Caetano                                                                                          
      //Lote, Validede, data de fabricação                                                      
      cItemPV	 := EE9_FATTIT

      EE7->( dbSetOrder( 1 ) )
      EE7->( dbSeek( xFilial( "EE8" ) + EE9->EE9_PEDIDO ) )
            
      SD2->( dbSetOrder( 8 ) )
      SD2->( dbSeek( xFilial( "SD2" ) + EE7->EE7_PEDFAT + cItemPV + EE9->EE9_COD_I ) )
      
      SF2->( dbSetOrder( 1 ) )
      SF2->( dbSeek( xFilial( "SF2" ) + SD2->D2_DOC + SD2->D2_SERIE + SD2->D2_CLIENTE + SD2->D2_LOJA + SD2->D2_FORMUL + SD2->D2_TIPO ) )
            
       
      Do While SD2->D2_PEDIDO == EE7_PEDFAT .and. SD2->D2_ITEM = cItemPV .and. SD2->D2_ITEM = EE9->EE9_COD_I
      
      		cNumLote	:= SD2->D2_NUMLOTE
      		cValidad	:= DtoC( SD2->D2_DTVALID )
    	  	cNumProd	:= Substr( cNumLote	,3	,3 )
      		dDatProd	:= DtoC( CtoD( "01/01" + "/" + Year( Date() ) ) + Val( Substr( cNumLote	,3	,3 ) ) )
      
      		cMemo += Chr(13) + nNumLote + " - " + cValidad + " - " + cNumProd + "-" + dDatProd
      		
      	    SD2->( dbSkip() )
      Enddo
      
      //Container
      DETAIL_P->AVG_C02_60 := SF2->F2_XCONTAI
