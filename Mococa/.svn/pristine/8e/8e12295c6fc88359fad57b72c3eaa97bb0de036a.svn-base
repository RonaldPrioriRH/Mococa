...
Private cEol := chr(13)+chr(10) 
	...
	cQry += "   AND CYV_XLOTE  = '"+StrZero(Val(cLote),3)+"' 				" +cEOL
	cQry += "   AND CYV_DTEO = ' '  				" +cEOL
	cQry += " ORDER BY R_E_C_N_O_ " +cEOL
	 // parei aqui !!!
	MemoWrite("C:\temp\CYV_Lote.sql",cQry)
	...
	DbGotop()
			...
			If TRBCYV->(!Eof()) 		
				While TRBCYV->(!Eof())
					 ...
					 If (TRBCYV->QTDOP) = 1
					 Help(,, 'Ordem de Produção' ,, 'Já existe a Ficha:' +cFicha+ ' e Palete:' +cPalet+ ' e Lote :	'+cLote+ ' cadastrados para este Apontamento ' + cOP +  ' !', 1,0) 
					 lRet := .F. 
