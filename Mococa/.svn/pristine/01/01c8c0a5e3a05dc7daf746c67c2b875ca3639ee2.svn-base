...
User Function RACDA014()
Local nOpc

If UsaCB0("01")
	xACDV0601(1)   // produto com cb0
Else
	VTCLear()
	@ 0,0 VTSay STR0001 // //'Selecione:'
	nOpc:=VTaChoice(2,0,4,VTMaxCol(),{"Nota de Entrada","Producao","Sem Documento"}) //### //"Nota de Entrada"###"Producao"###"Sem Documento"
	VtClearBuffer()
	If nOpc == 1
		xACDV061()  // produto sem cb0 (Nota entrada)
	ElseIf nOpc == 2
		xACDV062()  // produto sem cb0 (Producao)
	ElseIf nOpc == 3
		xACDV063()  // produto sem cb0 e sem informar o codigo do documento
	EndIf
EndIf
Return NIL


Static Function xACDV061()
xACDV0601(2)
Return NIL

Static Function xACDV062()
xACDV0601(3)
Return NIL

Static Function xACDV063()
If ! SIX->(DbSeek("SDA2"))
	VtAlert("Tabela"+CRLF+"SDA"+CRLF+"Ordem" +CRLF+"2"+CRLF+"Chave"+CRLF+"DA_FILIAL+DA_PRODUTO+DA_LOCAL+DA_LOTECTL","Indice obrigatorio") // "Tabela" ### "Ordem" ### "Chave" ### "Indice obrigatorio"	
	Return
EndIf	
xACDV0601(4)
Return NIL
...
Static Function xACDV0601(nTipo,lEnvCQ)
Local oTpTab1	:= NIL
Local oTpTab2	:= NIL
Local bkey09
Local bkey24
...
bkey09 := VTSetKey(09,{|| Informa()},"Informacoes") //"Informacoes"
...
While .t.
	VTClear
	If ! lCQ
		@ 0,0 VTSAY "Enderecamento" // //"Enderecamento"
	Else
		@ 0,0 VTSAY "Envio C.Q." //"Envio C.Q."
	EndIf
	nL := 0
	nQtdePro := 1
	If nTipo == 2 // nota
		@ 1,00 VTSAY  'Nota ' VTGet cNota   	pict '@!'          	when Empty(cNota)   .or. VtLastkey()==5 Valid VldNota(cNota) F3 cConsF3 // //'Nota '
		@ 1,14 VTSAY '-' VTGet cSerie       	pict '!!!'          when (Empty(cSerie) .and. lBranco) .or. VtLastkey()==5 Valid VldNota(@cNota,@cSerie,,,.T.)
		@ 2,00 VTSAY  'Forn.' VTGet cFornec 	pict '@!' F3 'FOR'	when Empty(cFornec) .or. VtLastkey()==5 Valid VldNota(cNota,cSerie,cFornec) // //'Forn.'
		@ 2,14 VTSAY '-' VTGet cLoja        	pict '@!'         	when Empty(cLoja)   .or. VtLastkey()==5 Valid (lBranco := .f.,VldNota(cNota,cSerie,cFornec,cLoja))
		@ 3,00 VTSAY "Qtde."  //"Qtde."
		@ 4,00 VTGet nQtdePro pict CBPictQtde()  valid nQtdePro > 0 when (lForcaQtd .or. VTLastkey() == 5)
		nL := 4
	ElseIf nTipo == 3 // producao
		@ 1,00 VTSAY 'Documento ' VTGet cDoc      pict '@!'          when Empty(cDoc) .or. VtLastkey()==5 Valid VldDoc(cDoc) // //'Documento '
		@ 2,00 VTSAY 'Qtde.' VTGet nQtdePro  pict CBPictQtde()  valid nQtdePro > 0 when (lForcaQtd .or. VTLastkey() == 5)//'Qtde.'
		nL := 2
	ElseIf nTipo == 4 
		@ 1,00 VTSAY "Armazem" VTGet cArmEti  pict "@!" Valid ! Empty(cArmEti)  // "Armazem"
		@ 2,00 VTSAY 'Qtde.' VTGet nQtdePro  pict CBPictQtde()  valid nQtdePro > 0 when (lForcaQtd .or. VTLastkey() == 5)//'Qtde.'
		nL := 2
	EndIf
	If UsaCB0("01") .and. UsaCB0("02")
		@ ++nL,0 VTSAY 'Etiqueta'  // //'Etiqueta'
		@ ++nL,0 VTGET cProd PICTURE "@!" VALID VTLastkey() == 05 .or. Empty(cProd) .or.  VldEtiq()
	Else
		@ ++nL,0 VTSAY 'Etiqueta: ' VTGET cProd PICTURE "@!" VALID VTLastkey() == 05 .or. Empty(cProd) .or. VldProd()
		@ ++nL,0 VTSAY 'Endereco'  // //'Endereco'
		If UsaCB0("02")
	...
	EndIf
	VTREAD
	lACD060CA  := ExistBlock("ACD060CA")
	If VTLASTKEY()==27
		If Empty(aDist) .or. VTYesNo("Saindo perdera o que foi lido, confirma saida?","ATENCAO",.T.)		  //### //"Saindo perdera o que foi lido, confirma saida?"###"ATENCAO"
			If lACD060CA
...
If ! SF1->(MsSeek(xFilial('SF1')+cNota+cSerie+cFornec+cLoja))
	VTBEEP(2)
	VTALERT("Nota nao encontrada","AVISO",.T.,3000)  //### //"Nota nao encontrada"###"AVISO"
	VTKeyBoard(chr(20))
	Return .f.
EndIf
If GetMv("MV_CONFFIS")=="S" .AND. GetMv("MV_TPCONFF",.F.,"1")=="2" .AND. SF1->F1_STATCON<>"1"
	VTBEEP(2)
	VTAlert("Nota " + AllTrim(SF1->F1_DOC) + "/" + AllTrim(SF1->&(SerieNfId("SF1",3,"F1_SERIE"))) + "STR0059",STR0015,.T.,4000)
	VTKeyBoard(chr(20))
	Return .f.
EndIf
...
For nP:= 1 to len(aItensPallet)
	...
	If ExistBlock("AIC060VPR") .and. ! ExecBlock("AIC060VPR",.F.,.F.,{cProd,aEtiqueta[2]*nQtdePro,aEtiqueta})
		Break
	EndIf
   ...
   If Empty(cNota+cSerie+cFornec+cLoja+cDoc) .and. ! UsaCB0("01") .And. CB0->CB0_ORIGEM <>"SD3" 
	...
	If UsaCB0("01")
		dbSelectArea("CB0")
	   	If	aEtiqueta[15] == cProd  
			dbSetOrder(2)
		Else
			dbSetOrder(1)	
		EndIf
		
		dbSeek(xFilial("CB0")+cProd)
		
		If !SimpleLock("CB0")
			VTALERT("Não foi possivel travar o produto",STR0015,.T.,3000)
			Break
	  	Endif
	EndIf 
	...
	For nX := 1 to len(aChavPesq)
		...
		Else
			aadd(aDist,{cChavPesq,aEtiqueta[1],aChavPesq[nX,2],aChavPesq[nX,3],{{cProd,aEtiqueta[2]}},cLote,cSLote,cNumseri})
		EndIf
...
If !Empty(aNumSeq) .AND. !Empty(aNF) .AND. GetMv("MV_CONFFIS")=="S" .AND. GetMv("MV_TPCONFF",.F.,"1")=="2"
	...
	For nX:=1 to Len(aNF)
		If SF1->(DbSeek(xFilial("SF1")+aNF[nX,01]+aNF[nX,02]+aNF[nX,03]+aNF[nX,04])) .AND. SF1->F1_STATCON<>"1"
			VTAlert("Nota: " + AllTrim(aNF[nX,01]) + "/" + AllTrim(aNF[nX,02]) + "STR0059","STR0015",.T.,4000)
			aNumSeq :={}
			Exit
		Endif
	Next
...
EndIf
If lCQ
	If ! Empty(GetMV("MV_CBENDCQ")) .and. ! Alltrim(cArmazem+cEndereco)+";" $ GetMV("MV_CBENDCQ")
		VTBEEP(2)
	...
	Else
		If UsaCB0("01")
			For nX := 1 to len(aDist[nI,5])
				...
				Else
					CBGrvEti("01",{NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,cLocaliz,cLocaL,,cNumSeq,NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,NIL,aDist[nI,8]},aDist[nI,5,nX,1]) // Atualiza o numero de serie
				EndIf
...
Else
	aCab  := {STR0029,STR0030,STR0039,STR0040,"Serie"}  //"Produto"###"Quantidade"###"Lote"###"SubLote" ### "Serie"
	aSize := {15,12,10,7,20}
	aHisEti:= {}
...
If CBChkSer(SB1->B1_COD) .And. Ascan(aDist,{|x| x[2]+x[8] == SB1->B1_COD+cNumseri}) > 0
	VTAlert("Esse numero de serie ja foi lido para esse produto",STR0015,.T.,3000)//##"Esse numero de serie ja foi lido para esse produto"##AVISO"
	lRet:=.F.
EndIf                                   
...
If ! SBE->(DbSeek(xFilial("SBE")+cArm))
	VTBeep(3)
	VTAlert('Armazem nao existe',"Armazem",.t.,4000) //'Armazem nao existe'###'Armazem'
	VTKeyBoard(chr(20))
	Return .f.
EndIf
