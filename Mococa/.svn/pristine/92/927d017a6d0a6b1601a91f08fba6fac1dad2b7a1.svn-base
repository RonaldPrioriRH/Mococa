...
±±ºPrograma  ³MACTBM02  ºAutor  ³Tarcísio Silva      º Data ³  02/02/18   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Limpa fleg da SE2 - Contas a Pagar.						  º±±
...
User Function MACTBM02() 
if !RetCodUsr()$GetMv("MV_XCTBSE2",,"000287")
	MsgAlert("Usuário sem premissão - MV_XCTBSE2!","Atenção")
	return()
endif
...
Private		aTypes     	:= {}
Aadd(aTypes,{ "Iif( !Eof(),E2_LA='S', .F. )"		,	"BR_VERDE" 	}	)
Aadd(aTypes,{ "Iif( !Eof(),(E2_LA='N' .OR. E2_LA=' '), .F. )"		,	"BR_VERMELHO" 	}	)
...
Private aRotina := { {"Pesquisar","AxPesqui",0,1} ,;
             {"Visualizar","AxVisual",0,2} ,;
             {"Sim/Nao","U_SIMNAOE2",0,3} ,;
             {"Legenda","U_LegAcert",0,4},;
             {"Processo em Massa","Processa({|| u_MACTBM06('SE2') },'Processando...')",0,5}}
...
Return
User Function SIMNAOE2

		reclock("SE2",.f.)
		replace E2_LA 	with IIF(E2_LA="S","N","S")
		msUnLock()
