...
If nTipo <> 1
	...
	Else
		...
		If lVerPerg
			if !lJob
				If lVerPerg .and. lVerPerg2 //Mococa/Gontijo
...
Else
	//Mococa/Gontijo FreeObj(oDANFE)
	//Mococa/Gontijo oDANFE := Nil	
EndIf
...
If nTipo <> 1
	...
	PRIVATE OFONT21N   := TFontEx():New(oDanfe,"Times New Roman",21,21,.T.,.T.,.F.)// 14
	PRIVATE OFONT14N   := TFontEx():New(oDanfe,"Times New Roman",14,14,.T.,.T.,.F.)// 15
...
Local lMv_Logod     := iif(GetMv("MV_LOGOD")$".T.",.T.,.F.)//Alterado por Marcos Godinho 30/07/2018 
...
oDanfe:Say(007, 542, iif(lNFCE,"NFC-e","NF-e"), oFont10N:oFont)
oDanfe:Say(022, iif(oDanfe:nDevice == 2,500,512 ), StrZero(Val(oIdent:_NNf:Text),9), oFont21N:oFont)
oDanfe:Say(033, 534, "SÉRIE "+SubStr(oIdent:_Serie:Text,1,3), oFont14N:oFont)
...
If lMv_Logod
	...
	cLogoD := GetSrvProfString("Startpath","") + "DANFE.BMP"//GetSrvProfString("Startpath","") + "DANFE" + cDescLogo + ".BMP" //Alterado por Marcos Godinho, todas as filiais imprimem a mesma logo com o mesmo nome de arquivo. 30/07/2018
