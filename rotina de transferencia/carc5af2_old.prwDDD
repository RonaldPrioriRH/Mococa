	...
	Else
		...
		If Pergunte(cPerg,.T.)
			...
			Else
				if MV_PAR02 <> 0
					If LockByName("CARC5AF2", .T., .T.)
						Processa({|| fProcTrf(MV_PAR02)}, "Processando...")
					Endif
				endif
			Endif
		EndIf
...
Static Function fProcTrf(nPar)
	...
	Private cTpOpEnt    := SUPERGETMV( "XX_OPTRFE", .T., "10") //Tipo de operação para escolha da TES
	Private cErroSef	:= ''
	Private nPrcVen		:= 0
