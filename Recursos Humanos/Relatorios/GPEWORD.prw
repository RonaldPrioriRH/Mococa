...
Local nAt			:= 0
Local lDepende		:= .T. //If (Mv_par26 = 1, .T., .F.)
...
nDepen				:= If ( !lDepende, 4,nDepende )
	...
	While SRA->( !Eof() .and. Eval( &(cInicio) ) <= cFim )
		...
		If !( SRA->RA_FILIAL $ fValidFil() .and. Eval( cAcessaSRA ) )
			...
			Loop
		EndIf
//Nádla Maria->> 18/06/2024
//desconsiderar pergunta MV_PAR26
		If lDepende
			If nDepende == 1 //Salario Familia
				...
				Else
					...
					// SRA->(dbSkip())
					// aAdd(aMsg,SRA->RA_MAT+" - " + SRA->RA_NOME + " - " + "Não foram encontradas informações de salário família de dependentes na SRB." )//"Não foram encontradas informações de salário família de dependentes na SRB."
					// Loop
				EndIf
			ElseIf nDepende == 2 //Imposto de Renda
				...
				Else
					...
					// SRA->(dbSkip())
					// aAdd(aMsg,SRA->RA_MAT+" - " + SRA->RA_NOME + " - " + "Não foram encontradas informações de IR de dependentes na SRB." )//"Não foram encontradas informações de IR de dependentes na SRB."
					// Loop
				EndIf
...
Local cNewPathArq	:= cGetFile( cTipo , STR0007, , , , nOR(GETF_MULTISELECT, GETF_NETWORKDRIVE, GETF_LOCALHARD) )									
	...
	If !Empty( cNewPathArq )
		If Len( cNewPathArq ) > 99
	    	MsgAlert( "O endereco completo do local onde está o arquivo do Word excedeu o limite de 99 caracteres!" ) // "O endereco completo do local onde está o arquivo do Word excedeu o limite de 75 caracteres!"
	    	Return			
		Else
...
Local cNewPathArq	:= ""
    ...
    // Verifica se o arquivo já foi selecionado através do zoom, caso contrário abre para seleção
    If Empty(MV_PAR25) 
        cNewPathArq := cGetFile( cTipo , STR0007, , , , nOR(GETF_MULTISELECT, GETF_NETWORKDRIVE, GETF_LOCALHARD))
    Else 
        cNewPathArq := MV_PAR25
    Endif


	If !Empty( cNewPathArq )
		If Len( cNewPathArq ) > 99
			MsgAlert( STR0187 ) // "O endereco completo do local onde está o arquivo do Word excedeu o limite de 99 caracteres!"
			Return
		Else
			If  Upper( Subst( AllTrim( cNewPathArq ), - 3 ) ) == Upper( AllTrim( STR0008 ) )
				Aviso( STR0009 , cNewPathArq , { STR0010 } )
...
Local cCodAtiv		:= ""
  ...
  //	aAdd( aExp, {'GPE_DESC_SEXO'   		   		,	SRA->(IF(RA_SEXO ="M","Masculino","Feminino"))			, "@!"						,STR0039	} )
	...
	If cPaisLoc <> "EQU"
       ...
       // <<<<<<< HEAD:Mococa/Recursos Humanos/Relatorios/GPEWORD.prw
...
//=======
//>>>>>>> fb2c24c967e48f802da64b4e488a1f9149737ed8:Fertminas/GPEWORD.prw
...
//=======
	aAdd( aExp, {'GPE_HRS_MENSAIS'		   		,	Transform(SRA->RA_HRSMES,'@E 999.99')				    , "SRA->RA_HRSMES"			,STR0071	} )
...
//>>>>>>> fb2c24c967e48f802da64b4e488a1f9149737ed8:Fertminas/GPEWORD.prw
    ...
    //Nádla Maria 22/08/2024 
	aAdd( aExp, {'GPE_SALARIO2'		   	 ,"R$ "+Alltrim(Transform(SRA->RA_SALARIO, X3Picture("RA_SALARIO"))), "@!"			            ,STR0095	} )
		nDias := SRA->(RA_VCTOEXP-RA_ADMISSA)+1
		cDiaExp := Extenso(nDias,.T.)
	aAdd( aExp, {'GPE_DIAS_EXPERIENCIA'		,	StrZero(nDias,02)				                        , "@!"						,STR0059	} )
	aAdd( aExp, {'GPE_EXT_DIAS_EXPERI'		,	cDiaExp      	        	                            , "@!"						, "Dias de Experiencia por Extenso"} )
	...
	aAdd( aExp, {'GPE_ANO_EXPERIENCIA2'		,	StrZero( Year( SRA->RA_VCTEXP2 ) , 4 ) 					, "@!"						,STR0248	} )
		nDias2 := SRA->(RA_VCTEXP2-RA_ADMISSA)+ 1
		cDiaExp2 := Extenso(nDias2,.T.)
		nDiasExp2 := SRA->(RA_VCTEXP2-RA_VCTOEXP)
		cDiaExper2 := Extenso(nDiasExp2,.T.)
		dBH:= If(MESDIA(SRA->RA_ADMISSA)>'1225'.and. (MESDIA(SRA->RA_ADMISSA)<='0625'),'25/06','25/12') 
	aAdd( aExp, {'GPE_DIAS_EXPERIENCIA2'	,	StrZero(nDias2, 02)			       	                    , "@!"						,STR0249	} )
	aAdd( aExp, {'GPE_DIAS_EXPER2'      	,	StrZero(nDiasExp2, 02)			       	                , "@!"						,STR0249	} )
	aAdd( aExp, {'GPE_EXT_DIAS_EXPER2'     	,	cDiaExper2          	       	                        , "@!"						,STR0249	} )
	aAdd( aExp, {'GPE_EXT_DIAS_EXPERI2'		,	cDiaExp2  	        	                                , "@!"						, "Dias de Experiencia 2 por Extenso"} )
	aAdd( aExp, {'GPE_DATA_EX_MEDIC'		,	SRA->RA_EXAMEDI											, "SRA->RA_EXAMEDI"			,STR0060	} )
	aAdd( aExp, {'GPE_MES_BH'		        ,	dBH														, "@!"						,STR0247	} )
	...
	aAdd( aExp, {'GPE_DESC_TP_CONT_TRAB'		,	(IF(SRA->RA_TPCONTR =="1","Indeterminado",( If( SRA->RA_TPCONTR == "2" ,"Determinado", (If( SRA->RA_TPCONTR == "3","Intermitente", "Não informado")))))), "@!"						,STR0118	} )
