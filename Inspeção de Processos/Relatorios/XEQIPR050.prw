...
User Function XEQIPR050() 
	...
	Private lProduto   := .F.
    Private lLabmic := .F.
    Private cIdioma := "Espanhol"//Portugês//Espanhol
    Private cGrupo := ""
...
Static Function A050Imp(lEnd,wnRel,cString,cTitulo)
	...
	For nContador := 1 To mv_par07
		...
		EndIf
		Li++
		@Li,01 PSAY STR0050+Replicate(".",23-Len(STR0050))+":"//Ordem de produção
			...
			QPK->QPK_OP+QPK->QPK_LOTE+QPK->QPK_NUMSER == QPR->QPR_OP+QPR->QPR_LOTE+QPR->QPR_NUMSER
				...
				If cLabor <> QPR->QPR_LABOR	
					...
					If lEnsaio
						...
						@Li,062 PSAY STR0058//Subs(TitSX3("QP7_UNIMED")[1],1,9)
						If mv_par01 ==1 .And. QP1->QP1_TPCART <> "X"
							...
							If dbSeek(xFilial("QP7")+QPR->QPR_PRODUT+QPR->QPR_REVI+cRoteiro+QPR->QPR_OPERAC+QPR->QPR_ENSAIO) .AND. QPR->QPR_OPERAC $ mv_par13
								If QP7->QP7_CERTIF == "S"
									...
									if len(CapitalAce(AllTrim(QP1->QP1_DESCIN))+"/"+CapitalAce(AllTrim(QP1->QP1_DESCPO)))>42
										...
										@Li,01 PSAY SubStr(CapitalAce(AllTrim(QP1->QP1_DESCES))+"/"+CapitalAce(AllTrim(QP1->QP1_DESCPO)),1,42)//u_funTraduz(QP1->QP1_DESCPO,cIdioma)//QP1->QP1_DESCPO
										
									else
										
										@Li,01 PSAY CapitalAce(AllTrim(QP1->QP1_DESCES))+"/"+CapitalAce(AllTrim(QP1->QP1_DESCPO))
									
									endif //CONTAGEM TOTAL//QP1->QP1_DESCPO
		...
		cQry := " SELECT B1_GRUPO  "
			...
			@Li,045 PSAY STR0047 //Metodos	
			@Li,085 PSAY STR0058 //Un.Medida
			@Li,110 PSAY STR0061 //Max		
			@Li,120 PSAY STR0060 //Resultado Final
			...
			cGrupo := AllTrim(TRB->B1_GRUPO)
				...
				While !ZZD->(Eof()) .AND. ZZD->ZZD_FILIAL == xFilial("ZZD") .AND. ZZD->ZZD_GRUPO == AllTrim(TRB->B1_GRUPO)
					...
					@Li,045 PSAY u_funTraduz(ZZD->ZZD_DESCSB,cIdioma) 						
					@Li,090 PSAY ZZD->ZZD_SIMBOL 						 
					@Li,109 PSAY TRANSFORM( ZZD->ZZD_VALOR,'@E 999.99' )
					@Li,130 PSAY ZZD->ZZD_VALOR2		
...
//***********************************************************************************		
			...
			Li++
			@Li,001 PSAY STR0064//Analista de Laboratório 						
		...
		If dbSeek(cSeek)
			...
			If dbSeek(xFilial("QPD")+QPL->QPL_LAUDO)
				Li+=2
				@Li,01 PSAY STR0067+Replicate(".",15-Len(STR0067))+": "+CapitalAce(QPD->QPD_DESCES)//u_funTraduz(QPD->QPD_DESCPO,cIdioma)
				Li++
		...
		@Li,01 PSAY "[ "+STR0062+" ]"
		Li++
		@Li,01 PSAY "[kg =  kilogramo/quilograma] [FQ = Físico-Químico/Físico-Quimico] [N.M.P. = Número más Probable/Número mais provavel]" 
		Li++
		@Li,01 PSAY "[Mín. = Mínimo/Mínimo] [µg = micrograma/micrograma]	[MB = Microbiología/Microbiologia] [Máx. = Máximo/Máximo]"
		Li++
		@Li,01 PSAY "[U.F.C = Unidade formadora de colonia/Unidade formadora de colonia] [Col. = Coliformes/Coliformes] [mg = miligramos/miligramas]"
		Li++
		@Li,01 PSAY "[N.D. =  No encontrado/Não Detectado] [cps = centipoise/centipoise] [ml = mililitro/mililitro] [µ = micro/micro]"
		Li++
		@Li,01 PSAY "[Un. = Unidad/Unidade] [% = Porcentaje/porcentagem] [g = gramos/gramas]"
		Li++			
	Next nContador
	Li+=2
	//Alert(str(Li))
