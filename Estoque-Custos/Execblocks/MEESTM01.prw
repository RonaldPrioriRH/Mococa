...
cQuery += " AND D3_OP BETWEEN '"+mv_par07+"' AND '"+mv_par08+"'
cQuery += " AND D3_CF='PR0' AND D3_ESTORNO<>'S' AND D3_OP NOT LIKE '%OS%' "
	...
	While SG1->(G1_FILIAL+G1_COD)=xFilial('SG1')+_OPS->D3_COD
		...
		if lOldModD3
			While SD3->(D3_FILIAL+D3_NUMSEQ)=xFilial('SD3')+_OPS->D3_NUMSEQ
				if !SD3->D3_TIPO$cTipos .OR. (SD3->D3_TIPO='MO' .AND. SD3->D3_CHAVE='E0') //Só MOD
					DbSkip()
					Loop
				endif
				dbSelectArea("SD3")
...
//Retirado Sangelles 22/03/2022 ->SX1->( dbSetOrder(1) )
cPerg 	:=	PADR(cPerg,10)
...
u_xPutSx1(     cPerg,"07","Ordem de Produção de?",".",".","mv_ch7","C",14,0,0,"G","",""   	,"","","mv_par07","","","","","","","","","","","","","","","","")
u_xPutSx1(     cPerg,"08","Ordem de Produção Ate?",".",".","mv_ch8","C",14,0,0,"G","",""   	,"","","mv_par08","","","","","","","","","","","","","","","","")
