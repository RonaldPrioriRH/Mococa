...
If Alltrim(ReadVar())= "M->E2_CODBAR"
	If Empty(M->E2_CODBAR)
		...
		//Trata Ferriados e Fds
		//Claudio 02.01.18
		dDataRef:=dDataBase
		While !IsDiaUtil(dDataRef-1)
		   dDataRef--
		Enddo
		//verifica se estแ vencido
		if SUBSTR(cCodbarras,6,4) <> "0000" 
			if DateDiffDay( dDataRef,ctod("07/10/1997 ")) > val(SUBSTR(cCodbarras,6,4)) 
...
ฆฆ+-----------------------------------------------------------------------+ฆฆ
ฆฆฆPrograma  ฆ DiasUteis   ฆ Autor ฆ Claudio Ferreira          ฆ 28/12/07 ฆฆฆ
ฆฆ+----------+------------------------------------------------------------ฆฆฆ
ฆฆฆDescri็เo ฆ Funcao para retornar o numero de dias uteis do mes da data ฆฆฆ
ฆฆ+----------+------------------------------------------------------------ฆฆฆ
ฆฆฆ Uso      ฆ Generico                                                   ฆฆฆ
ฆฆ+-----------------------------------------------------------------------+ฆฆ
ฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆฆ
ฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏฏ
*/
Static Function DiasUteis(_dData)
	Local _DIAI,_DIAF,_DIAS

	_DIAI := FirstDay(_dData)
	_DIAF := LastDay(_dData)
	_DIAS=0
	DO WHILE _DIAI <= _DIAF
		IF IsDiaUtil(_DIAI)
			_DIAS++
		ENDIF
		_DIAI++
...
Static Function IsDiaUtil(_dData)
	Local _lRet := .F.
	Local _FERIADOS:="01/01-21/04-01/05-19/06-07/09-12/10-02/11-15/11-25/12"  //Por default jแ considera os feriados nacionais fixos
	
	IF DOW(_dData)#7 .AND. DOW(_dData)#1 .AND. (!SUBSTR(DTOC(_dData),1,5)$_FERIADOS) .AND. !FeriadoSP3(_dData) 
		_lRet := .T.
	ENDIF
Return _lRet

Static Function FeriadoSP3(_dData)
	Local _Ret:=.f.
	_sAlias	:=	Alias()
	dbSelectArea("SP3")
	dbSetOrder(1)
	if dbSeek( xFilial( 'SP3' ) + dtos(_dData) )  //Busca o casual
		_Ret:=.t.
	else
		dbSetOrder(2)
		if dbSeek( xFilial( 'SP3' ) + substr(dtos(_dData),5,4) )  //Busca o fixo
			_Ret:=.t.
		endif
	endif
