...
/*/
//--------------------------------------------------------------------
User Function UPDZPA( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDZPA" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDZPA" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX2, { ;
	'ZPA'																	, ; //X2_CHAVE
	cPath																	, ; //X2_PATH
	'ZPA'+cEmpr																, ; //X2_ARQUIVO
	'IMPORTACAO FOLHA PCO'													, ; //X2_NOME
	'IMPORTACAO FOLHA PCO'													, ; //X2_NOMESPA
	'IMPORTACAO FOLHA PCO'													, ; //X2_NOMEENG
	'E'																		, ; //X2_MODO
	''																		, ; //X2_TTS
	''																		, ; //X2_ROTINA
	''																		, ; //X2_PYME
...
aAdd( aSX2, { ;
	...
	'DEPARA IMP PCO'														, ; //X2_NOME
	'DEPARA IMP PCO'														, ; //X2_NOMESPA
	'DEPARA IMP PCO'														, ; //X2_NOMEENG
	'C'																		, ; //X2_MODO
	''																		, ; //X2_TTS
	''																		, ; //X2_ROTINA
	''																		, ; //X2_PYME
...
aAdd( aSX3, { ;
	{ 'ZPA'																	, .T. }, ; //X3_ARQUIVO
	{ '03'																	, .T. }, ; //X3_ORDEM
	{ 'ZPA_CLASSE'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Classe Orc'															, .T. }, ; //X3_TITULO
	{ 'Classe Orc'															, .T. }, ; //X3_TITSPA
	{ 'Classe Orc'															, .T. }, ; //X3_TITENG
	{ ''																	, .T. }, ; //X3_DESCRIC
	{ ''																	, .T. }, ; //X3_DESCSPA
	{ ''																	, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
...
aAdd( aSIX, { ;
	'ZPA'																	, ; //INDICE
	'1'																		, ; //ORDEM
	'ZPA_FILIAL+DTOS(ZPA_DATA)+ZPA_SEQ+ZPA_CONTA+ZPA_CC'					, ; //CHAVE
	'Data+Seq+Conta+C.Custo'												, ; //DESCRICAO
	'Data+Seq+Conta+C.Custo'												, ; //DESCSPA
	'Data+Seq+Conta+C.Custo'												, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
aAdd( aSIX, { ;
	'ZPB'																	, ; //INDICE
	'1'																		, ; //ORDEM
	'ZPB_FILIAL+ZPB_CONTA+ZPB_CLORC'										, ; //CHAVE
	'Conta+Cl Orc'															, ; //DESCRICAO
	'Conta+Cl Orc'															, ; //DESCSPA
	'Conta+Cl Orc'															, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'N'																		} ) //SHOWPESQ
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDZPA" ) ) ) ;
